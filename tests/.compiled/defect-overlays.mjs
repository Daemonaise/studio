function computeEdgeDefects(geometry) {
  ...
  const openEdgeCoords = [];
  const nonManifoldCoords = [];
...
function computeSliverTriangles(geometry, threshold = 20) {
  const posAttr = geometry.getAttribute("position");
  if (!posAttr) return [];
  const positions = posAttr.array;
  const vertexCount = posAttr.count;
...
function computeInvertedNormals(geometry) {
  const posAttr = geometry.getAttribute("position");
  if (!posAttr) return [];
  const positions = posAttr.array;
  const vertexCount = posAttr.count;
  ...
  for (const faces of edgeFaces.values()) {
    ...
    neighborCount[f0]++;
    neighborCount[f1]++;
    const dot = faceNormals[f0 * 3] * faceNormals[f1 * 3] + faceNormals[f0 * 3 + 1] * faceNormals[f1 * 3 + 1] + faceNormals[f0 * 3 + 2] * faceNormals[f1 * 3 + 2];
    if (dot < -0.1) {
...
function computeExtendedDefects(geometry, sliverThreshold = 20) {
...
export {
  computeEdgeDefects,
  computeExtendedDefects,
  computeInvertedNormals,
  computeSliverTriangles
};
